...
public class BookRepositoryImpl implements BookRepository {
	...
	public Set<Book> getBookListByFilter(Map<String, List<String>> filter) {
		...
		Set<String> cirterias = filter.keySet();

		if (cirterias.contains("publisher")) {
			for (int j = 0; j < filter.get("publisher").size(); j++) {
				String publisherName = filter.get("publisher").get(j);
				String SQL = "SELECT count(*) FROM book where b_publisher like'%" + publisherName + "%'";
				booksByPublisher.addAll(template.query(SQL, new BookRowMapper()));
			}
		}

		if (cirterias.contains("category"))

		{
			for (int i = 0; i < filter.get("category").size(); i++) {
				String category = filter.get("category").get(i);
				String SQL = "SELECT count(*) FROM book where b_category like'%" + category + "%'";
